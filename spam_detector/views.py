...
def index(request):
    if request.method == 'POST':
        ...
        if len(message) == 0:
            messages.error(request, "Please enter some message to check.")
            ...
            if model.predict(message):
                messages.error(request, "Be Careful! This is a SPAM Message.")
                return redirect('index')
            else:
                messages.success(request, "Wow! This is not a SPAM Message.")
                return redirect('index')   
